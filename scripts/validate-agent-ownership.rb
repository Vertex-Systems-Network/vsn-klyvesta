...
def branch_name
  ENV['AGENT_HEAD_BRANCH'].to_s.strip.then do |value|
    return value unless value.empty?
  end

  ENV['GITHUB_HEAD_REF'].to_s.strip.then do |value|
    return value unless value.empty?
  end

  ENV.fetch('GITHUB_REF_NAME', '').strip
end
...
def changed_files
  ...
  base_ref = ENV.fetch('AGENT_BASE_REF', 'origin/main')
