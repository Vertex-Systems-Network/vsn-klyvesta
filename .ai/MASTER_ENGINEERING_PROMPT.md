...
README progress/status is mandatory durable delivery state. After every accepted integration, the Supervisor must update README `Last status update` in the same bounded closeout path. When accepted integration or terminal closeout changes canonical lifecycle/progress/timeline/public-delivery truth, the same reviewed change must update the repository-owned lane count/progress bar and every affected Module delivery table row. A delivery-changing milestone is not fully closed while README is stale. If accepted code is merged but lifecycle closeout remains pending, README must explicitly report that pending state and must not prematurely advance the canonical percentage. Governance/security-only cycles that do not change accepted delivery truth update compact/governance state without fabricating README progress.
